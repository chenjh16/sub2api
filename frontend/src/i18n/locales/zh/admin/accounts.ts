export default {
    accounts: {
      ...
      expandToolbar: '展开工具栏',
      collapseToolbarHint: '隐藏账号操作、筛选和批量编辑栏，最大化列表高度',
      expandToolbarHint: '显示账号操作、筛选和批量编辑栏',
      autoRefresh: '自动刷新',
      ...
      openai: {
        ...
        modelsSyncHeadersTitle: '模型同步请求头',
        modelsSyncHeadersDesc:
          '默认关闭，关闭时使用 Sub2API 原本的 /v1/models 同步请求。仅影响后台模型同步，不影响普通转发。',
        modelsSyncUserAgentMode: 'User-Agent 模式',
        modelsSyncUserAgentDefault: '默认浏览器 UA',
        modelsSyncUserAgentCustom: '自定义 UA',
        modelsSyncUserAgentDefaultHint:
          '开启后默认使用内置浏览器 UA；如上游需要特殊请求头，可切换为自定义 UA 或补充 Headers JSON。',
        modelsSyncUserAgent: '模型同步 User-Agent',
        modelsSyncUserAgentPlaceholder: 'Mozilla/5.0 ...',
        modelsSyncUserAgentHint: '如果上游边缘策略要求特定浏览器 UA，可在这里覆盖默认值。',
        modelsSyncHeaders: '模型同步 Headers（JSON）',
        modelsSyncHeadersHint:
          '填写 JSON 对象。Host、Content-Length、Connection 等逐跳头会被后端忽略；通常不要配置 Authorization。',
        modelsSyncHeadersInvalid: '模型同步 Headers 必须是 JSON 对象，且值只能是字符串、数字或布尔值。',
      ...
      modelAutoRefreshTitle: '自动刷新模型列表',
      modelAutoRefreshDescription: '按账号定时拉取上游 /models，并更新当前模型候选列表。',
      modelAutoRefreshToggle: '开启或关闭自动刷新模型列表',
      modelAutoRefreshInterval: '刷新间隔（分钟）',
      modelAutoRefreshIntervalHint: '最小 10 分钟，最大 30 天。',
      modelAutoRefreshTestFilter: '拉取后自动测试并过滤不可用模型',
      modelAutoRefreshTestFilterHint: '开启后会按全局批量测试并发度探测模型，只把测试成功的模型加入实际可用列表。',
      autoRuleMapModels: '自动规则映射',
      autoMapModels: '自动映射',
      autoMapNoModels: '当前没有可分析的模型',
      autoMapNoChanges: '没有发现可追加的模型映射',
      autoMapAdded: '已更新 {count} 条模型映射，并同步全局规则白名单',
      autoRuleMapAdded: '已按全局规则白名单更新 {count} 条模型映射',
      autoMappingSettingsLoadFailed: '读取模型映射自动化设置失败',
      autoMappingSettingsSaveFailed: '更新模型映射规则白名单失败',
      batchTestModels: '批量测试模型',
      batchTestingModels: '正在测试...',
      batchTestModelsDone: '测试 {success}/{total}，失败 {failed}',
      modelBatchTestResults: '模型测试结果',
      testThisModel: '测试该模型',
      modelTestTotal: '总数',
      modelTestSuccess: '成功',
      modelTestFailed: '失败',
      ...
      modelTestQueued: '等待测试',
      modelTestImageReceived: '已收到图片响应',
      modelTestRequiresSavedAccount: '需要先保存账号后才能测试模型',
      modelTestNoResponseBody: '测试接口没有返回响应体',
      modelTestStreamEnded: '测试流已结束，但没有收到完成事件',
      removeModel: '移除该模型',
      enableModel: '启用该模型',
      disableModel: '停用该模型',
      ...
      enabledModelCount: '已启用 {enabled} / 候选 {total}',
      poolMode: '池模式',
      ...
      textPromptPlaceholder: '例如：你是什么模型？',
      textPromptDefault: '你是什么模型？',
      textTestHint: '会使用当前选择的文本模型发送该提示词，用于验证账号连接。',
