export default {
    settings: {
      ...
      site: {
        ...
        modelMappingAutomationTitle: '模型映射自动化',
        modelMappingAutomationDescription: '管理账号弹窗中“自动规则映射”的全局白名单，并设置批量测试模型的默认并发度',
        modelBatchTestConcurrency: '批量测试并发度',
        modelBatchTestConcurrencyHint: '用于账号编辑窗口的批量模型测试，范围 1-10，默认 3',
        modelMappingAutoRules: '自动命名规则白名单',
        modelMappingAutoRulesHint: '启用的规则按“请求模型 → 上游实际模型”应用；“自动映射”发现的新规则也会追加到这里',
        addModelMappingRule: '添加规则',
        saveModelMappingAutomation: '保存自动化设置',
        loadModelMappingAutomationFailed: '读取模型映射自动化设置失败',
        saveModelMappingAutomationSuccess: '模型映射自动化设置已保存',
        saveModelMappingAutomationFailed: '保存模型映射自动化设置失败',
        modelMappingFromPlaceholder: '请求模型，如 deepseek-v4-pro',
        modelMappingToPlaceholder: '上游实际模型，如 DeepSeek-V4-Pro',
        modelMappingRuleSourcePlaceholder: '来源说明',
        modelMappingAutoRulesEmpty: '暂无规则；点击“添加规则”手动维护，或在账号弹窗中使用“自动映射”追加',
        customEndpoints: {
          title: '自定义端点',
      ...
      gatewayFailoverPolicy: {
        ...
        descriptionLabel: '描述',
        statusCodes: '状态码',
        excludeStatusCodes: '排除状态码',
        statusRanges: '状态码范围',
        jsonConditionGroup: 'JSON 条件组',
        headerConditionGroup: 'Header 条件组',
        messageConditionGroup: 'Message 条件组',
        bodyConditionGroup: 'Body 条件组',
        transportConditionGroup: '网络错误条件组',
        maxScanBytes: '最大扫描字节',
        jsonLogic: 'JSON 条件逻辑',
        headerLogic: 'Header 条件逻辑',
        jsonConditions: 'JSON 条件',
        headerConditions: 'Header 条件',
        messageConditions: 'Message 条件',
        bodyConditions: 'Body 条件',
        transportConditions: '网络错误条件',
        transportPersistent: '网络错误类型',
        any: '任意',
        transient: '瞬时',
        persistent: '持久',
        consecutive: '连续窗口',
        ...
        jsonRulesRequired: 'JSON 必须是规则数组，或包含 rules 数组的对象',
        jitterPercent: '冷却抖动（%）',
        threshold: '触发阈值',
