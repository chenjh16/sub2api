...
func (s *AccountTestService) testOpenCodeGoAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, locale string) error {
	...
	switch proto {
	case APIProtocolAnthropic:
		return s.testCNProviderAnthropicConnection(c, account, testModelID, prompt, locale)
	case APIProtocolResponses:
		return s.testOpenCodeGoResponsesConnection(c, account, testModelID, prompt, locale)
	default:
		return s.testCNProviderChatCompletionsConnection(c, account, testModelID, prompt, locale)
	}
}

func (s *AccountTestService) testOpenCodeGoResponsesConnection(c *gin.Context, account *Account, testModelID string, prompt string, locale string) error {
...
func (s *AccountTestService) testGrokAccountConnection(c *gin.Context, account *Account, modelID, prompt, mode string, opts AccountTestOptions) error {
	...
	case AccountTestModeGrokText:
		...
		return s.testGrokResponsesConnection(c, ctx, account, authToken, testModelID, prompt, opts.Locale)
	}
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID, prompt, locale string) error {
	...
	payloadBytes, err := buildGrokAccountTestResponsesBody(testModelID, prompt, locale)
	if err != nil {
...
func buildGrokAccountTestResponsesBody(model, prompt, locale string) ([]byte, error) {
	if strings.TrimSpace(prompt) == "" && strings.TrimSpace(locale) == "" {
		return buildGrokQuotaProbeBody(model)
	}
	return json.Marshal(map[string]any{
		"model":  strings.TrimSpace(model),
		"input":  normalizeTextTestPromptForLocale(prompt, locale),
		"stream": true,
	})
}
